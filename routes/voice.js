...
exports.interview = function(request, response) {
    ...
    SurveyResponse.advanceSurvey({
        ...
        if (err || !surveyResponse) {
            say('Terribly sorry, but an error has occurred.');
            return respond();
        }

        // If question is null, we're done!
        if (!question) {
            say('Transaction completed on Blockchain.');
            return respond();
        }

        // Add a greeting if this is the first question
        if (questionIndex === 0) {
            say('Low cost payment solution for rural India');
        }
...
exports.transcription = function(request, response) {
    console.log('Transcription called in voicejs')
    ...
    console.log('Transcription request is - '+request.questionIndex)
    console.log('Transcription response is - '+response)
